...
The following residue-theoretic identity provides an alternative
expression for the Fisher information in terms of critical points
rather than roots.
While not strictly necessary for the general proof, it offers
additional insight and yields closed-form expressions in low degree.

\begin{theorem}[Critical-point identity]\label{thm:critval}
...
To illustrate the main result and provide concrete verification,
we now examine the cases $n=2$ and $n=3$ explicitly.
...
\begin{remark}[Hermite polynomial characterization]\label{rem:hermite}
The equality condition $V_i = c(\lambda_i-\bar\lambda)$ characterizes,
up to affine transformation, the zeros of the Hermite
polynomial~$H_n$.

Indeed, the Hermite polynomial $H_n$ satisfies the differential equation
$H_n''-2xH_n'+2nH_n=0$.
Evaluating at a zero~$x_k$ of~$H_n$ yields $H_n''(x_k)=2x_k H_n'(x_k)$.
Dividing both sides by $2H_n'(x_k)$ and applying
Lemma~\ref{lem:score-deriv} (which states $V_k = H_n''(x_k)/(2H_n'(x_k))$)
gives $V_k = x_k$.
Since the zeros $x_1,\ldots,x_n$ of~$H_n$ are centered
(i.e., $\bar x := \frac{1}{n}\sum_i x_i = 0$), we have
$V_k = x_k = x_k - \bar x = x_k - 0$.
Conversely, any centered real-rooted polynomial satisfying
$V_i = c(\lambda_i-\bar\lambda)$ for a constant~$c$ has roots that,
after affine rescaling, coincide with the zeros of~$H_n$.

For $n=2$ this holds for all distinct root configurations.
For $n=3$, the equality condition forces the third-order coefficient~$T$ to vanish, consistent with Theorem~\ref{thm:stam3}.
\end{remark}
...
\begin{remark}[Equality characterization]\label{rem:equality}
