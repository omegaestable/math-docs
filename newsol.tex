...
and discriminant $\Delta = 4S^3-27T^2 > 0$.
...
= \frac{72S\alpha^2}{\Delta}
= \frac{72S^2/3}{\Delta}\cdot 3
= \frac{72S^2}{\Delta}$.
\end{proof}
...
For $n=2$ this holds for all distinct root configurations; for $n=3$
it reduces to $T=0$, consistent with Theorem~\ref{thm:stam3}.
\end{remark}
...
\begin{proof}
Divide the expansion of Lemma~\ref{lem:phi-change} by $h$ and let
$h \to 0$.
\end{proof}
...
\ge 0$.
Integrate from $0$ to $1$ and identify the endpoints.
\end{proof}
...
For $n=2$ every polynomial satisfies this; for $n=3$
it reduces to $T_1=T_2=0$ (Theorem~\ref{thm:stam3}).
\end{remark}
