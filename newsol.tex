...
\noindent
We prove~\eqref{eq:stam} in full for $n=2$ (Section~\ref{sec:n2}) and
$n=3$ (Section~\ref{sec:n3}), and reduce the general case to a single
pointwise inequality (Section~\ref{sec:general}).
...
For centered~$p$ $($i.e.\ $\bar\lambda=0$$)$,
...
% =====================================================================
\section{General \texorpdfstring{$n$}{n}: reduction to a pointwise inequality}
\label{sec:general}
% =====================================================================

\begin{conjecture}[Pointwise score--gap inequality]
\label{conj:pointwise}
For every $r\in\PnR$ with distinct roots,
\begin{equation}\label{eq:pw}
\mathcal{S}(r)\,\sigma^2(r)\;\ge\;\frac{n-1}{2}\,\Phi_n(r).
\end{equation}
Equality holds for $n=2$ (always) and for $n=3$ when $r(x)=x^3-Sx$.
\end{conjecture}

\noindent
Conjecture~\ref{conj:pointwise} has been verified numerically for
$3\le n\le 8$ over thousands of root configurations (random, clustered,
geometric, and outlier arrangements). The minimum observed ratio
$\mathcal{S}\sigma^2\big/\bigl(\tfrac{n-1}{2}\Phi_n\bigr)$ was
$\ge1.000$ in every test.

\begin{theorem}[Conditional Stam inequality]\label{thm:cond-stam}
If Conjecture~\ref{conj:pointwise} holds for degree~$n$,
then the Stam inequality~\eqref{eq:stam} holds for degree~$n$.
\end{theorem}
...
\begin{tabular}{c|l|l}
$n$ & Status & Equality condition \\\hline
$2$ & Proved (equality) & Always \\
$3$ & Proved (strict unless trivial) & $T_1=T_2=0$ \\
$\ge4$ & Conditional on Conj.~\ref{conj:pointwise}
      & Expected: strict
\end{tabular}
\end{center}

\subsection*{Key new tools}
\begin{enumerate}
\item \textbf{Critical-value formula} (Theorem~\ref{thm:critval}):
$\Phi_n=-\frac{1}{4}\sum_j r''(\zeta_j)/r(\zeta_j)$,
relating Fisher information to the critical structure of the
polynomial.

\item \textbf{Case-split argument} (Theorem~\ref{thm:cond-stam}):
the pointwise inequality~\eqref{eq:pw} plus the integral
identity, applied from both the $p$-flow and $q$-flow directions,
yields the full Stam inequality via a dichotomy on the
product $\sigma^2\Phi_n$.
\end{enumerate}

\subsection*{What remains}
Proving Conjecture~\ref{conj:pointwise} for $n\ge4$ would complete
the proof for all degrees. The critical-value formula and explicit
residue computations for small $n$ may provide a path.

% =====================================================================
\section{Proof of Conjecture~\ref{conj:pointwise} for \texorpdfstring{$n=4$}{n=4}}
\label{sec:n4}
% =====================================================================

% ----- PROOF STRATEGY NOTES (to be cleaned up) -----
%
