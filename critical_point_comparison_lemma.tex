...
For a centered polynomial $r$ with roots $-\lambda_n < \cdots < -\lambda_1 < 0 < \lambda_1 < \cdots < \lambda_n$ 
(symmetric about 0), each critical point $\zeta_j$ satisfies
\[
...
The critical values of $H_n$ are related to $H_{n-1}$ by the recurrence 
$H_n' = 2nH_{n-1}$, giving
\[
  H_n(\zeta_j) = \frac{H_n^2(\zeta_j)}{H_n(\zeta_j)} 
  = \frac{(H_n'(\zeta_j))^2}{2nH_{n-1}(\zeta_j)\cdot H_n(\zeta_j)} 
  = 0
\]
...
\[
  ...
  = -\frac{4n(n-1)}{3\sigma^2(H_n)} 
  = -\frac{8n(n-1)}{3(n-1)} 
  = -\frac{8n}{3},
\]
confirming the optimal constant.
