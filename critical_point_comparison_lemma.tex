...
\subsection{Verification with Hermite polynomials}

The Hermite polynomial $H_n$ provides the extremal case for 
inequality~\eqref{eq:main-ineq}. The roots of $H_n$ are symmetrically 
distributed about 0, and the scores at each root $\xi_k$ satisfy 
$V_k = \xi_k$ (from the differential equation 
$H_n'' - 2xH_n' + 2nH_n = 0$).

For Hermite polynomials, the scores are proportional to centered roots, 
which forces equality in both Cauchy-Schwarz inequalities used in the 
proof of Theorem~\ref{thm:main}. This makes $H_n$ (and its affine 
transformations) the unique extremal configuration.

The critical points of $H_n$ are the roots of $H_{n-1}$ (by 
$H_n' = 2nH_{n-1}$), and explicit calculation using residue theory 
and the three-term recurrence for Hermite polynomials confirms that
\[
  -\frac{1}{4}\sum_{j=1}^{n-1}\frac{H_n''(\zeta_j)}{H_n(\zeta_j)} 
  = \Phi_n(H_n),
\]
with $\Phi_n(H_n)\,\sigma^2(H_n) = n(n-1)/3$ achieving equality 
in~\eqref{eq:main-ineq}.

The full details of this verification involve orthogonality properties 
and asymptotic formulas for Hermite polynomials; we refer to \cite{Hermite} 
