...
Higher coefficients are $O(\epsilon^3)$ or smaller.
The operator $T_{q_\epsilon} = \sum_{k} \frac{(n-k)!}{n!}\,b_k\,\partial_x^k$ therefore acts as:
\[
T_{q_\epsilon}\,p(x) = p(x) + \frac{(n-2)!}{n!}\,b_2\,p''(x) + O(\epsilon^3)
...
0 = p(\lambda_i) + \delta_i\,p'(\lambda_i) + O(\delta_i^2)
    - \frac{\epsilon^2}{2(n-1)}\,p''(\lambda_i) + O(\epsilon^3).
\]
...
K_{q_t}(z) \coloneqq K_q(z)^{\,t}, \qquad t\in[0,1],
\end{equation}
where $K_q(z)^{\,t}$ is expanded as a power series and truncated at degree $n$.
Then $K_{q_0}(z)=1$ (so $q_0=x^n$), $K_{q_1}(z)=K_q(z)$, and by construction
$K_{q_s}(z)K_{q_t}(z)=K_{q_{s+t}}(z)$, which yields the semigroup property
...
\begin{remark}
Although $K_q(z)^{\,t}$ is a power series when $t$ is non-integer, only the first $n$ coefficients matter when acting on degree-$n$ polynomials, because $\partial_x^{n+1}$ annihilates them. Thus the definition of $q_t$ via~\eqref{eq:Kt} is unambiguous at the operator level, and the semigroup identity follows from~\eqref{eq:K-product}. In what follows we assume $p_t$ has distinct real roots for $t\in[0,1]$ so that $\mathcal{S}(p_t)$ is well-defined; this is guaranteed for small steps by continuity of roots and by preservation of real-rootedness under $\boxplus_n$.
\end{remark}
