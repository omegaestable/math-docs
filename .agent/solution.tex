...
In each small step $k$, we start with some polynomial $p_{k-1}$ (with roots
$\lambda_1^{(k-1)},\dots,\lambda_n^{(k-1)}$ and scores $V_i^{(k-1)}$) and convolve
it with $q_k$.  Lemma~\ref{lem:fisher_decrease} gives the decrease
\[
\Phi_n(p_k)
= \Phi_n(p_{k-1})
  - \frac{2\epsilon^2}{n-1}
    \sum_{i<j}\frac{\bigl(V_i^{(k-1)}-V_j^{(k-1)}\bigr)^{2}}
                    {\bigl(\lambda_i^{(k-1)}-\lambda_j^{(k-1)}\bigr)^{2}}
  + O(\epsilon^4).
\]
Write $S_k = \sum_{i<j}(V_i^{(k-1)}-V_j^{(k-1)})^2/(\lambda_i^{(k-1)}-\lambda_j^{(k-1)})^2$
for the non-negative ``score-gradient'' at step $k$.

\emph{Step 2c: Strictness.}
Why is the inequality strict for $n \ge 3$?  The quantity
$S_k = \sum_{i<j}(V_i-V_j)^2/(\lambda_i-\lambda_j)^2$ measures the rate
at which Fisher information decreases under convolution.  By the final
step in the proof of Lemma~\ref{lem:fisher_decrease}, $S_k > 0$ whenever
$n\ge 3$ and the roots are distinct (since the scores cannot all be equal).
Hence at \emph{every} infinitesimal step the Fisher information drops by
strictly more than the Stam bound would require, and the accumulated
excess makes the final inequality strict.
