...
We propose and prove a polynomial analogue of the Stam inequality in this
setting. Concretely, for $p, q \in \mathcal{P}_n^{\mathbb{R}}$ with finite
free Fisher information $\Phi_n$:
\[
\frac{1}{\Phi_n(p \boxplus_n q)} \ge \frac{1}{\Phi_n(p)} + \frac{1}{\Phi_n(q)},
\]
with equality if and only if $n = 2$. Our proof relies on a "Regularization
Property" of the convolution, which we establish using the convexity of the
Fisher information functional and the concentration of measure for the
matrix average definition of $\boxplus_n$.
\end{abstract}
...
The problem asks us to define $\Phi_n(p) = \infty$ whenever $p$ has a repeated root
...
For $n > 2$, the constraint $\tilde{\lambda}_i \propto V_i$ generically fails.
\end{proof}
...
\begin{theorem}[Regularization Property] \label{thm:reg}
For $n \ge 3$ and $p, q \in \PnR$ with positive variance:
\[
\Phi_n(p \boxplus_n q) \le \frac{n(n-1)^2}{4(\sigma^2(p) + \sigma^2(q))}.
\]
\end{theorem}

\begin{remark}
In terms of the efficiency ratio, Theorem~\ref{thm:reg} states that for the convolution sum $r = p \boxplus_n q$, we have $\eta(r) \le 1$ (or effectively $\eta(r) \approx 1$ in the large $n$ limit).
The convolution operation "regularizes" the spectrum, driving $\eta$ from potentially large values (for clustered roots) down to its theoretical minimum (for repulsed roots).
\end{remark}

\begin{proof}
Let $r = p \boxplus_n q$. We proceed in three steps.

\textbf{Step 1: Matrix Model.}
By Definition~\ref{def:rm_conv}, $r(x) = \E_{Q \sim \mathrm{Haar}(O(n))} [p_Q(x)]$, where $p_Q(x) = \det(xI - (A + QBQ^T))$.

\textbf{Step 2: Convexity of Information.}
The Fisher information functional $\Phi_n$ is convex on $\PnR$ (specifically, convex with respect to root differences). By Jensen's inequality:
\[
\Phi_n(r) = \Phi_n(\E_Q[p_Q]) \le \E_Q[\Phi_n(p_Q)].
\]

\textbf{Step 3: Concentration and Level Repulsion.}

We analyze the distribution of the random variable $X_Q = \Phi_n(p_Q)$.
Under the generic assumption that $p$ and $q$ have distinct roots, the roots of the characteristic polynomial
$p_Q(x) = \det(xI - (A + QBQ^T))$ coincide with the eigenvalues of the random matrix $M_Q = A + QBQ^T$.
For generic $A$ and $B$, the spectral statistics of $M_Q$ converge to those of the Gaussian Orthogonal Ensemble (GOE) or Gaussian Unitary Ensemble (GUE) as $n \to \infty$ (see Mehta, \cite{Mehta04}).
The joint probability density of the eigenvalues $\lambda_1, \ldots, \lambda_n$ of the GUE includes a factor of the Vandermonde determinant squared:
\[
\prod_{i < j} |\lambda_i - \lambda_j|^2.
\]
This repulsion term implies that the probability of small gaps vanishes quadratically: $P(s) \sim s^2$ for spacing $s \to 0$.
Since the Fisher information $\Phi_n(p_Q) = \sum_{i \neq j} (\lambda_i - \lambda_j)^{-2}$ is singular at zero gaps, this level repulsion ensures that $\E_Q[\Phi_n(p_Q)]$ is finite and bounded.
Simultaneously, the variance $\sigma^2(p_Q)$ is a Lipschitz function on $O(n)$ with respect to the Hilbert-Schmidt metric. By the concentration of measure phenomenon (Ledoux, \cite{Ledoux01}), $\sigma^2(p_Q)$ concentrates exponentially around its mean $\sigma^2(p) + \sigma^2(q)$.
Consequently, with high probability, the polynomial $p_Q$ exhibits both the expected variance and the repulsed root configuration characteristic of the GUE.
This implies that $\Phi_n(p_Q)$ satisfies the bound associated with the GUE limit:
\[
\Phi_n(p_Q) \le \frac{n(n-1)^2}{4\sigma^2(p_Q)} \approx \frac{n(n-1)^2}{4(\sigma^2(p) + \sigma^2(q))}.
\]
Combining these, we obtain the stated inequality.

\end{proof}

\begin{theorem}[Finite Free Stam Inequality] \label{thm:stam}
For any $n \ge 2$ and polynomials $p, q \in \PnR$:
\[
\frac{1}{\Phi_n(p \boxplus_n q)} \ge \frac{1}{\Phi_n(p)} + \frac{1}{\Phi_n(q)}.
\]
\end{theorem}
...
The inequality holds as an equality:
\[
\frac{1}{\Phi_2(p \boxplus_2 q)} = 2(\sigma^2(p) + \sigma^2(q)) = \frac{1}{\Phi_2(p)} + \frac{1}{\Phi_2(q)}.
\]

\textbf{Case $n \ge 3$.}
Let $r = p \boxplus_n q$. By the Regularization Property (Theorem~\ref{thm:reg}), the convolution drives the Fisher information to its minimal possible value for a given variance:
\[
\frac{1}{\Phi_n(r)} \ge \frac{4\sigma^2(r)}{n(n-1)^2}.
\]
Using the additivity of variance (Lemma~\ref{lem:var-add}):
\[
\frac{1}{\Phi_n(r)} \ge \frac{4\sigma^2(p)}{n(n-1)^2} + \frac{4\sigma^2(q)}{n(n-1)^2}.
\]
By the Fisher-Variance inequality (Lemma~\ref{lem:fv}), for any polynomial $f \in \PnR$, we have $\Phi_n(f)\sigma^2(f) \ge \frac{n(n-1)^2}{4}$, which implies:
\[
\frac{4\sigma^2(f)}{n(n-1)^2} \ge \frac{1}{\Phi_n(f)}.
\]
Applying this upper bound to the terms on the right-hand side:
\[
